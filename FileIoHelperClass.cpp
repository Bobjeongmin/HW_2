...
)
{
	...
	{
		printf("no file exists. file=%ws", FilePath.c_str());
...
#pragma warning(disable: 4127)
	...
	do
	{
		...
		{
			printf("[ERR ] CreateFile(%ws) failed, gle=0x%08x", FilePath.c_str(), GetLastError());
			break;
		}
		...
		{
			printf("[ERR ] %ws, can not get file size, gle=0x%08x", FilePath.c_str(), GetLastError());
			break;
		}
		...
		if (NULL == mFileMap)
		{
			printf("[ERR ]CreateFileMapping(%ws) failed, gle=0x%08x", FilePath.c_str(), GetLastError());
			break;
		}
...
#pragma warning(disable: 4127)
	...
	do
	{
		...
		{
			printf("SetFilePointerEx( move to %I64d ) failed, gle=0x%08x", FileSize.QuadPart, GetLastError());
			break;
		}
...
)
{
	...
	if (TRUE != DT_SUCCEEDED(status))
	{
		printf("FIOReference() failed. status=0x%08x", status);
		return status;
	}
	...
	{
		printf("exception. code=0x%08x", GetExceptionCode());
